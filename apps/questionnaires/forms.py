...
questions = dict((x.question_slug,x.question_text) for x in Question.objects.all())
yes_no_choices = (('Yes','Yes'),('No','No'))

class QuestionnaireForm(forms.Form):
    imp_issue = forms.CharField(max_length=280,label=questions['imp_issue'])
    rent_ctrl = forms.MultipleChoiceField(label=questions['rent_ctrl'],choices=yes_no_choices)
